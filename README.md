...
- Next.js 서버가 시작되면 `src/instrumentation.ts`에서 `codex app-server`를 stdio 자식 프로세스로 실행해 자동 연결을 시도한다. 웹 UI는 연결 완료를 기다리지 않고 시작하며, 초기화와 계정 확인을 마치면 사이드바에 연결 상태를 표시한다. 실패하거나 수동으로 해제한 경우 **연결** 버튼으로 다시 연결할 수 있다.
...
- 연결 상태는 SSE 스트림으로 최초 상태와 변경 사항을 전달한다. 주기적으로 조회하지 않으며, 스트림이 끊어지면 브라우저가 재연결한다. 화면에는 UI만 두고 클라이언트 연결 훅·상태 스트림·자동 연결 처리는 `src/lib/codex/`에서 관리한다. `src/instrumentation.ts`는 Next.js 시작 진입점이다.
- 현재 구현은 단일 로컬 Node.js 서버 기준이다. 서버를 재시작하면 다시 자동 연결을 시도한다. 연결 상태 확인은 모델 생성 요청을 보내지 않으며, 콘텐츠 생성 연결은 다음 작업이다.
